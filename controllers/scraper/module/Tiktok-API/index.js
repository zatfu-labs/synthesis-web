		...
		axios
			...
			.then((res) => {
				...
				main.each(function () {
					...
					const thumb = $('#user-page > div.user.container > div > div.col-md-3.col-4.my-3 > div')
						.attr('style')
			...
			.then((resp) => {
				...
				pickrandom(Vidlink).then((res) => {
					axios.get(res).then((respon) => {
						...
						resolve({
							url: ch('#tiktok-video-result > div > div.download-buttons > div > a')
								.attr('href')
							...
							nowm:
								...
								ch('#tiktok-video-result > div > div.download-buttons > div > a')
									.attr('href')
									.replace('https://savetiknowm.org/?tiktok_url=', ''),
